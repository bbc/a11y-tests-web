...
const getDefaultLighthouseConfig = () => ({
  config: {
...
function run() {
  ...
  return executeSequentially(tasks)
    .then(() => {
      ...
      const jsonPath = getOutputJsonPath();
...
function getScoresPerCategory(reportCategories = [], url) {
  const results = reportCategories.map(({ name, score }) => ({ name, score }));
      ...
      auditKeys.forEach((auditKey) => {
        ...
        if (score !== true && !manual) {
          ...
          if (erroredElements.length > 0) {
            logErrorDetailsAsTestFailures(testCase, erroredElements, errorMessage);
          }
        }
      });
      return results;
...
function getLighthouseConfig(config) {
  const lighthouseConfig = getDefaultLighthouseConfig();

  if (config.hasOwnProperty('lighthouseCategories')) {
    lighthouseConfig.config.settings.onlyCategories =
      config.lighthouseCategories;
  }
...
function getOutputJsonPath() {
  const outputJsonPath = process.env.ATW_OUTPUT_JSON;

  if (!outputJsonPath) {
    logger.log('No JSON output path provided. Use the ATW_OUTPUT_JSON environment variable to set one.');
...
function logErrorDetailsAsTestFailures(testCase, erroredElements, errorMessage) {
