...
async function waitForInspectableTarget(chromeInstance) {
  ...
  return new Promise((resolve, reject) => {
    interval = setInterval(async () => {
      console.log('Waiting for inspectable target');

      if (attempts > 0) {
        console.log('Failed to get inspectable target, trying again...');
      }

      if (attempts > 50) {
        clearInterval(interval);
        reject('Failed to find target after 50 attempts');
      }
      ...
      try {
        ...
        if (isReady) {
          console.log('Inspectable target found');
          clearInterval(interval);
          resolve();
        }
      } catch(err) {
...
async function runTestsForUrl({ url, shouldSignIn, hide, isPretty }) {
  ...
  try {
    for (let i = 0; i < 100; i++) {
      ...
      let client = await external.CDP({ port: chrome.port });
