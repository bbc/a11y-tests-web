...
function runTestsForUrl({ url, shouldSignIn, hide, isPretty }) {
  ...
  return launchChromeAndRunLighthouse(url, shouldSignIn)
    .then((results) => {
      logger.log('START2 results');
      logger.log(results, typeof results);
      logger.log('END2 results');
      logger.log('START2 results.audits');
      logger.log(results.audits);
      logger.log('END2 results.audits');
      logger.log('START2 JSON.stringify(results.audits)');
      logger.log(JSON.stringify(results.audits));
      logger.log('END2 JSON.stringify(results.audits)');
    ...
    .then((chrome) => {
      return completeSetupSteps(chrome, url, shouldSignIn)
        .then(() => {
          ...
          return external.lighthouse(url, lighthouseFlags, lightHouseConfig).catch((err) => logger.error('issue invoking lighthouse external dep', err));
        })
        .then((results) => chrome.kill()
          .then(() => {
            logger.log('START results.audits');
            logger.log(results.audits);
            logger.log('END results.audits');
            logger.log('START JSON.stringify(results.audits)');
            logger.log(JSON.stringify(results.audits));
            logger.log('END JSON.stringify(results.audits)');
            return results;
          }).catch((err) => logger.error('Chrome kill threw an error', err))
        ).catch((err) => {
          logger.error('About to kill chrome', chrome);
          chrome.kill();
...
function signInToBBCID(chrome, url) {
  return external.CDP({ port: chrome.port })
    ...
    .catch((err) => logger.error('Failed to sign in to BBC ID', err));
}
