...
function run() {
  ...
  return executeSequentially(tasks)
    .then(() => {
      ...
      fs.writeFileSync(__dirname + '/../lighthouse-report.json', outputJson);
    ...
    .then((results) => {
      ...
      auditKeys.forEach((auditKey) => {
        ...
        if (score !== true && !manual) {
          ...
          // a11y, PWA and SEO categories
          const a11yPwaSeo = erroredElements.filter((e) => e.type === 'node' || e.any === null);

          // performance and best practices categories
          const perfBestPractices = erroredElements.filter((e) => e.type !== 'node' || e.any !== null);

          if (a11yPwaSeo.length > 0) {
            const errorDetail = getA11ySeoPwaErrorDetail(erroredElements);
            testCase.failure(errorMessage + errorDetail);
          }

          if (perfBestPractices.length > 0) {
            console.log('errored elements:', perfBestPractices);
            const errorDetail = getPerfBestPracticeErrorDetail(erroredElements);
            testCase.failure(errorMessage + errorDetail);
          }
        }
      });
...
function getPerfBestPracticeErrorDetail(elements) {
