...
function run() {
  ...
  const isTerser = process.env.A11Y_TERSER === 'true';
  ...
  const signedOutTasks = paths.map((path) => () => runTestsForUrl({ url: baseUrl + path, shouldSignIn: false, hide, isTerser }));
  const signedInTasks = signedInPaths.map((path) => () => runTestsForUrl({ url: baseUrl + path, shouldSignIn: true, hide, isTerser }));
...
function runTestsForUrl({ url, shouldSignIn, hide, isTerser }) {
