    ...
    .then((results) => {
      ...
      auditKeys.forEach((auditKey) => {
        ...
        if (score !== true && !manual) {
          ...
          const errorDetail = getErrorDetail({ details, extendedInfo, hide }) || '';
...
function getErrorDetail({ details, extendedInfo, hide }) {
  if (details.items) {
    const nonHiddenElements = details.items.filter(({ selector }) => !selectorIsHidden(selector, hide));
    return 'Failing elements:\n' + nonHiddenElements.map(({ selector, snippet }) => selector + ' - ' + snippet).join('\n');
  }
  if (extendedInfo.value && extendedInfo.value.nodes) {
    const nonHiddenElements = extendedInfo.value.nodes.filter(({ target }) => !selectorIsHidden(target[0], hide));
    return 'Failing elements:\n' + nonHiddenElements.map(({ target, snippet }) => target[0] + ' - ' + snippet).join('\n');
  }
}
