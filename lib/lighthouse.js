...
const console = require('./colourfulLog');
const {
  getConfigName,
  getConfig,
  getSignedInPaths,
...
function run() {
  ...
  if (!paths.length && !signedInPaths.length) {
    console.error(`No paths listed in the config for ${configName}`);
    process.exit(1);
  }
  ...
  console.log(`Tests will run against: ${domain} ${paths.join(' ')}`);
  if (signedInPaths.length) {
    console.log(`Tests will run signed in against: ${domain} ${signedInPaths.join(' ')}`);
  }
...
function runTestsForUrl({ url, shouldSignIn, hide, isPretty }) {
  ...
  return launchChromeAndRunLighthouse(url, shouldSignIn)
    .then((results) => {
      console.log('START2 results');
      console.log(results, typeof results);
      console.log('END2 results');
      console.log('START2 results.audits');
      console.log(results.audits);
      console.log('END2 results.audits');
      console.log('START2 JSON.stringify(results.audits)');
      console.log(JSON.stringify(results.audits));
      console.log('END2 JSON.stringify(results.audits)');
    ...
    .then((chrome) => {
      return completeSetupSteps(chrome, url, shouldSignIn)
        .then(() => {
          ...
          return external.lighthouse(url, lighthouseFlags, lightHouseConfig).catch((err) => console.error('issue invoking lighthouse external dep', err));
        })
        .then((results) => chrome.kill()
          .then(() => {
            console.log('START results.audits');
            console.log(results.audits);
            console.log('END results.audits');
            console.log('START JSON.stringify(results.audits)');
            console.log(JSON.stringify(results.audits));
            console.log('END JSON.stringify(results.audits)');
            return results;
          }).catch((err) => console.error('Chrome kill threw an error', err))
        ).catch((err) => {
          console.error('About to kill chrome', chrome);
          chrome.kill();
          console.error('Could not run lighthouse', err);
        });
    }).catch((err) => {
      console.error('Could not launch Chrome', err);
    });
}
...
function signInToBBCID(chrome, url) {
  return external.CDP({ port: chrome.port })
    ...
    .catch((err) => console.error('Failed to sign in to BBC ID', err));
}
